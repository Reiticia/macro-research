...
| AI 分析 | 填了个人 Key：设备直连模型，数据不经服务端；未填 Key 但有后端地址：按 provider 身份读取服务端共享分析（冻结；有访问令牌时可反馈） | 服务端为每个事件生成 3 种语言 × 3 种方法共 9 条分析并冻结；管理员经 Telegram 批准后才重新生成 |
...
- Binance 用于 BTC、ETH。
- BiQuote 优先用于指数、贵金属、能源、美元指数和 EUR/USD（能源使用 `USOIL` / `XNGUSD`），失败后回退 Yahoo。
- **美债收益率（`us2y` / `us10y`）以 CNBC 为主源**：报价接口取最新值与上一交易日收盘，`ts-api.cnbc.com` 的 1D 图提供约三个交易日的**1 分钟** OHLC，既做基准也做 1/5/15/30/60 分钟反应窗口。请求携带 CNBC 网页 `Referer`，避免 Akamai 返回 403。CNBC 直连在部分网络同样可走设置页代理；报价失败时依次回退 Yahoo（`^TNX` / `^UST2YR`）和美国财政部官方日频 CSV，官方日频值会标记为 stale。
- 直连模式在进程内缓存每个标的的当前价格和涨跌幅，页面每 5 秒刷新；同一标的的并发请求合并，失败时短期展示带 `stale` 标记的最后值。设备侧一次并发更新全部到期标的，不做服务端使用的错峰限流。
- Yahoo 用于指数、贵金属与美债的回退源。
...
- **未填 Key 但保存了后端地址**：通过免鉴权、只读的 provider 身份接口读取服务端在行情采集窗口结束后自动生成的共享分析。每个事件缓存 en / zh-CN / zh-TW 与三种方法的笛卡尔积，共 9 条。结果对所有人一致且冻结；配置了访问令牌后才可提交反馈（同一结果只记一条），由管理员在 Telegram 上决定「重新分析」或「忽略」，用户无法触发重新生成。
...
2. 事件名称译名统一来自服务端：后端模式下译名随事件返回；直连模式下客户端只需已配置的后端地址，即可通过免鉴权、只读的 `POST /api/v1/translations/names` 批量拉取（最多 100 个/批），覆盖列表里所有可见事件，不消耗任何模型额度，也不会触发翻译模型。访问令牌仍只用于日历、行情与分析等数据接口。未配置后端地址且未配置个人 Key 时，事件名保持英文原文；服务端译名请求失败会在「设置 → 数据来源」的后端卡片里显示原因，而不是静默显示英文。
