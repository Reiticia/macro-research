...
2. 事件名称译名统一来自服务端：后端模式下译名随事件返回；直连模式下客户端也会用已配置的后端地址与令牌，通过 `POST /api/v1/translations/names` 批量拉取（最多 100 个/批），覆盖列表里所有可见事件，不消耗任何模型额度。未配置后端且未配置个人 Key 时，事件名保持英文原文；服务端译名请求失败会在「设置 → 数据来源」的后端卡片里显示原因，而不是静默显示英文。
