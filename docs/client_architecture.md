...
- **美债收益率（`us2y` / `us10y`）以 CNBC 为主源**：报价接口取最新值与上一交易日收盘，`ts-api.cnbc.com` 的 1D 图提供约三个交易日的**1 分钟** OHLC，既做基准也做 1/5/15/30/60 分钟反应窗口。请求携带 CNBC 网页 `Referer`，避免 Akamai 返回 403。CNBC 直连在部分网络同样会被 TLS 拦截，因此它与 Yahoo 一样可走设置页的代理。Yahoo（`^TNX` / `^UST2YR`）保留为回退。
