...
2. 事件名称的中文显示同样零配置：客户端内置一份常用指标中文词典作为显示兜底（约 100 条，含简繁两式）；配置了后端时，优先使用服务端词典或 AI 译名（服务端会随事件返回，也可通过 `POST /api/v1/translations/names` 批量拉取）；配置了个人 Key 的直连模式则由本机 AI 翻译长尾名称。三层来源的优先级：人工勘正 > 服务端/本机译名 > 内置词典 > 英文原文。
