...
| AI 分析 | 用户自己的 OpenAI 兼容 Key，在设备端组装 payload 与解析回包 | 服务端用管理员 Key 生成并缓存 |
...
1. 未配置 Key（且非后端模式）：应用启动时强制 English，中文选项禁用。
2. 已配置 Key，或处于后端模式：解锁简体中文和繁体中文（后端模式下事件名由服务端提供，无需个人 Key）。
...
5. 删除 Key：立即切回 English，并锁定中文选项（除非处于后端模式）。
