...
| AI 分析 | 填了个人 Key：设备直连模型，数据不经服务端；未填 Key 但有后端地址：按 provider 身份请求服务端惰性分析（有访问令牌时可反馈） | 首次请求时只生成所请求的语言与方法并缓存；管理员经 Telegram 批准后才重新生成 |
...
- **未填 Key 但保存了后端地址**：通过带访问令牌的 provider 身份接口请求服务端共享分析。事件已公布且对应语言/方法没有缓存时才调用模型，后续直接读取数据库；用户请求不会为其他语言或方法预生成结果。配置了访问令牌后可提交反馈（同一结果只记一条），由管理员在 Telegram 上决定「重新分析」或「忽略」，用户无法触发重新生成。
